...
-   There are 603 observations (student rows with the same data for
    each).

-   Columns of different data come in different forms (numbers,
    characters, logic)
...
-   student cases or school cases

What columns typically (or you think may) represent:

-   the variables collected for each of the students or schools
...
-   If the cut-off is higher, more students are excluded, lessening the
    total number of rows in the tibbledd
...
```{r}
ggplot(d, aes(x = total_points_possible, y = total_points_earned)) +
  geom_point()
```
